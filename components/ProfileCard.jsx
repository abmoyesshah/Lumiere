...
/**
 * Tinder-style full-bleed profile card — purple/fuchsia theme.
 */
    ...
    >
      ...
      <div className={`relative ${isFull ? "aspect-[3/4.2] sm:aspect-[3/4]" : "aspect-[3/4]"}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-7 text-white">
          ...
          {isFull && bio && (
            <p className="text-sm text-white/75 mt-3 line-clamp-2 leading-relaxed font-light">
              {bio}
            </p>
          )}
          ...
          {isFull && interests?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-4">
              {interests.slice(0, 4).map((interest, i) => (
                <span key={i}
                  className="text-[10px] uppercase tracking-[0.18em] text-white bg-white/10 backdrop-blur-md border border-white/20 px-3 py-1 rounded-full font-medium">
                  {interest}
                </span>
              ))}
            </div>
        ...
        <div className="p-4 sm:p-5 border-t border-white/[0.06]" style={{ background: "rgba(15,8,28,0.6)" }}>
          <div className="flex flex-wrap gap-1.5">
            {interests.slice(0, 3).map((interest, i) => (
              <span key={i}
                className="text-[10px] uppercase tracking-[0.15em] text-white/70 border border-white/15 px-2.5 py-1 rounded-full font-medium">
                {interest}
              </span>
            ))}
          </div>
      ...
      {showPassLike && (
        <div className="p-5 sm:p-6 border-t border-white/[0.06] flex items-center justify-center gap-5"
          style={{ background: "rgba(15,8,28,0.7)" }}>
          <motion.button
            whileHover={{ scale: 1.08, y: -2 }} whileTap={{ scale: 0.92 }}
            onClick={(e) => { e.stopPropagation(); onPass?.(); }}
            className="w-16 h-16 sm:w-[68px] sm:h-[68px] rounded-full border border-white/20 hover:border-white/40 bg-white/5 hover:bg-white/10 text-white/80 flex items-center justify-center transition shadow-[0_8px_24px_-8px_rgba(0,0,0,0.6)] backdrop-blur-md"
            aria-label="Pass"
          >
          ...
          <motion.button
            ...
            className="w-14 h-14 rounded-full border border-fuchsia-400/30 bg-fuchsia-500/10 text-fuchsia-300 hover:bg-fuchsia-500/20 flex items-center justify-center transition backdrop-blur-md"
