...
const ProfileCard = memo(function ProfileCard({
  ...
  return (
    <motion.div whileHover={whileHover} transition={{ type: "spring", stiffness: 220, damping: 22 }}
      className={`${sizing} rounded-[2rem] overflow-hidden cursor-pointer border border-white/10 group`}
      ...
      <div className={`relative ${isFull ? "aspect-[3/4.2] sm:aspect-[3/4]" : "aspect-[3/4]"}`}>
        <SafeImage src={picture} name={name} alt={name} rounded="rounded-none" className="w-full h-full"
          bgClassName="bg-violet-950/50" textClassName="text-fuchsia-300 text-7xl sm:text-8xl font-light" iconSize={56} loading="eager" />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent pointer-events-none" />
        <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-black/40 to-transparent pointer-events-none" />
        {matchScore && (
          <div className="absolute top-4 right-4 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/15"
        ...
        <div className="absolute top-4 left-4 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-white/15"
          ...
          {isFull && interests?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-4">
              {interests.slice(0, 4).map((interest, i) => (
                <span key={i} className="text-[10px] uppercase tracking-[0.18em] text-white border border-white/20 px-3 py-1 rounded-full font-medium"
      ...
      {!isFull && interests?.length > 0 && (
        <div className="p-4 sm:p-5 border-t border-white/[0.06]" style={{ background: "linear-gradient(180deg, #0f081c, #1a0f2e)" }}>
      ...
      {showPassLike && (
        <div className="p-5 sm:p-6 border-t border-white/[0.06] flex items-center justify-center gap-5"
          style={{ background: "linear-gradient(180deg, #0f081c, #0a0612)" }}>
          <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
            onClick={(e) => { e.stopPropagation(); onPass?.(); }}
            className="w-16 h-16 sm:w-[68px] sm:h-[68px] rounded-full border border-white/20 hover:border-white/40 text-white/80 flex items-center justify-center"
            style={{ background: "linear-gradient(135deg, #1a0f2e, #0f081c)" }}>
            <X size={26} strokeWidth={2} />
          </motion.button>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.92 }}
            onClick={(e) => e.stopPropagation()}
            className="w-14 h-14 rounded-full border border-fuchsia-400/30 text-fuchsia-300 flex items-center justify-center"
            style={{ background: "linear-gradient(135deg, #d946ef, #a855f7)" }}>
            <Star size={20} strokeWidth={2} className="fill-fuchsia-300" />
          </motion.button>
          <motion.button whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
            onClick={(e) => { e.stopPropagation(); onLike?.(); }}
            className="w-20 h-20 sm:w-[88px] sm:h-[88px] rounded-full text-white flex items-center justify-center"
