...
export default function MatchListItem({ match, selected, onSelect, onViewProfile }) {
  return (
    <div
      className={`group relative w-full px-4 sm:px-5 py-3.5 cursor-pointer transition-all border-l-2 ${
        selected
          ? "bg-fuchsia-500/[0.06] border-fuchsia-400"
          : "border-transparent hover:bg-white/[0.03]"
      }`}
    >
      <div className="flex items-center gap-3.5">
        <div className="relative flex-shrink-0"
          onClick={(e) => { e.stopPropagation(); onViewProfile(match); }}>
          <SafeImage
          ...
          />
          <span className="absolute bottom-0.5 right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-400 ring-2 ring-[#0f0820] shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
        </div>
