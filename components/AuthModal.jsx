...
export default function AuthModal({
  isOpen, onClose, onAuthSuccess, initialStateIsLogin = true,
}) {
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 backdrop-blur-2xl overflow-y-auto"
          ...
          >
            ...
            >
              <div className="relative rounded-3xl overflow-hidden border border-white/10"
                ...
                {/* Glow blob */}
                <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-80 h-80 rounded-full opacity-30 blur-3xl pointer-events-none"
                ...
                <div className="relative px-8 sm:px-10">
                  <div className="flex p-1 rounded-full bg-white/[0.04] border border-white/[0.08]">
                    {[
                      ...
                      const active = isLogin === t.key;
                      return (
                        <button
                          key={String(t.key)}
                          type="button"
                          onClick={() => setIsLogin(t.key)}
                          className={`relative flex-1 py-2.5 text-xs font-semibold tracking-wide transition rounded-full z-10 ${
      ...
      <AnimatePresence>
        {submitting && (
          <LoadingSpinner variant="overlay" label={isLogin ? "Signing you in" : "Creating account"} />
        )}
      </AnimatePresence>
    </>
  );
