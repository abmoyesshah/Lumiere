...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`/api/profile/${id}`).then(r => r.ok ? r.json() : null).then(data => {
      if (data?.user) { setProfileData(data.user); setTimelinePhotos(data.user.timelinePhotos || []); }
    ...
    <div className="min-h-screen w-full overflow-x-hidden" style={{ background: "#0a0612" }}>
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-6">
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <button onClick={() => router.back()} className="text-white/50 hover:text-[#d946ef] text-[10px] sm:text-[11px] uppercase tracking-[0.25em] font-light flex items-center gap-1">
